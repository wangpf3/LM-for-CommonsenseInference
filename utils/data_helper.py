...
class DataHelper(object):
    ...
    def __init__(self, task, rel2token, tokenizer, max_seq_length, sample_ratio=0, seed=0, seen_tails=None):
        ...
        if not os.path.exists(cache_features_path):
            processor = DataProcessor(data_dir, rel2token, tokenizer, max_seq_length, sample_ratio, seen_tails)
        ...
        self.trainset = TensorDataset(trainset['input'], trainset['label'])
        self.devset = TensorDataset(devset['input'], devset['label'])
        # self.testset = TensorDataset(testset['input'], testset['label'])


class DataHelper_Test(object):
    """docstring for DataHelper"""
    def __init__(self, task, rel2token, tokenizer, max_seq_length, seen_tails=None, include_seen=False):

        data_dir = os.path.join('./data/', task)
        processor = DataProcessor(data_dir, rel2token, tokenizer, max_seq_length, 0, seen_tails, include_seen)
        testset = processor.get_tensors('test')
